...
class User extends MY_Controller{
    ...
    public function info($id = 0){
        ...
        {
            $user_info = $this->Muser->get_one('id, phone_number, portrait, auth_status, is_limit', [
                'id' => $user_id 
            ]);
            ...
            {
                $data['user_info'] = array_merge($user_detail, $user_info);
            }
            else
            {
    ...
    public function login(){
        ...
        $result = $this->Muser->login($user_phone, $password);
        if (!$user_phone || empty($password))
        {
            $this->return_failed('', array('msg' => '登录参数错误!', 'code' => 2));
        }
        
        $user_info = $this->Muser->get_one('*', array('phone_number' => $user_phone));
        if (! $user_info)
        {
        ...
        //更新登录时间
        $this->Muser->update_info(array('update_time' => date("Y-m-d H:i:s", time())), array('phone_number' => $user_phone));
    ...
    public function reg(){
     $data = $this->input->post('data');
        ...
        if(!preg_match(C('regular_expression.user_phone'), $data['user_phone']))
        {
            $this->return_failed('', array('msg' => '手机号格式不对！', 'code' => 3));
        }
        
        $check_tel = $this->Muser->get_one(array('id','portrait'), array('phone_number' => $data['user_phone']));
        if($check_tel)
        {
    ...
    public function detail(){
        try {
            ...
            if(empty($id) && empty($user_phone) && empty($disable))
            {
                throw new Exception('参数错误');
            }
            $where = array();
