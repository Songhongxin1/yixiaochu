...
class File extends MY_Controller {
    ...
    public function upload(){
        $file_dir = $this->input->get_post('type') == 'image' ? 'image/' : 'files/';
        $config = array(
        ...
        if ( ! $this->upload->do_upload('file')){
            ...
            echo json_encode(array('error' => 0, 'url' => '/uploads/'.$file_dir.$data['file_name'], 'full_url' => C('domain.img.url').'/'.$file_dir.$data['file_name']));
        }
        exit();
    }
