...
class Publicservice extends MY_Controller{
    ...
    public function mobile_code(){
        ...
        try {
            ...
            $check_tel = $this->Muser->get_one(array('id'), array('user_phone' => $mobile));
            if($check_tel)
            {
                $this->return_failed($status_msg_arr['4']);
            }
            ...
            if ($result)
            {
                $send_return = send_msg($mobile, '验证码为：' . $code . $sms_config['waring']);
                if($send_return)
                {
