...
class News extends MY_Controller{
    ...
    public function batch_publish(){
        ...
        $update_data['update_user'] = $_SESSION['USER']['id'] ? $_SESSION['USER']['id'] : 0;
        $update_data['update_time'] = date('Y-m-d H:i:s');
        $result = $this->Mnews->update_info($update_data, array('in' => array('id' => $ids)));
        if ($result) {
            $this->return_json(array('msg' => '批量发布成功'));
        } else {
            $this->return_json(array('msg' => '批量发布失败'));
        }
    ...
    public function edit($id) {
        ...
        if (IS_POST) {
            ...
            //保存文章资讯
            unset($post_data['file'], $post_data['SEO_title'], $post_data['SEO_keywords'], $post_data['SEO_description'], $post_data['rich_text_img']);
            $post_data['publish_status'] = 0; //修改之后把状态置为0，等待确认发布
            $post_data['parent_class_id'] = $this->Mnews_class->get_one('parent_id', array('id' => $post_data['news_class_id']))['parent_id'];
    ...
    public function update_status($id, $state) {
        $res = $this->Mnews->update_info(array('publish_status' => $state), array('id' => $id));
        if ($res) {
            $this->success("操作成功！", "/news");
        } else {
            $this->success("操作失败！请重试！", "/news");
    ...
    public function class_list() {
        ...
        //父级分类
        $data['parent_lists'] = $this->Mnews_class->get_lists("id, name", array('parent_id' => 0));
    ...
    public function comment(){
      ...
      $data['lists'] = $this->Mnews_comment->get_lists('*', 0, $order_by);
      
      $data_count = count($data['lists']);
      $data['data_count'] = $data_count;
      $data['page'] = $page;
      
      $pageconfig['base_url'] = "/news/comment?";
      $pageconfig['total_rows'] = $data_count;
      $this->pagination->initialize($pageconfig);
      $data['pagestr'] = $this->pagination->create_links(); // 分页信息
      
      //获取文章标题
      $article_id = array_column($data['lists'], 'news_id');
      if($article_id){
          $query['in'] = array('id'=>$article_id);
          $article_lists = $this->Mnews->get_lists('id, title', $query);
          $data['article_lists'] = array_column($article_lists, 'title', 'id');
      }
    ...
    public function com_del($id= 0){
        $id = intval($id);
        $where['id'] = $id;
        $is_del = $this->Mnews_comment->get_one('is_del', array('id'=>$id))['is_del'];
        $del = $this->Mnews_comment->update_info(array('is_del'=>!$is_del), $where);
        if($del){
            $this->success('操作成功', '/news/comment');
        }else{
            $this->success('操作失败', '/news/comment');
        }
    }
