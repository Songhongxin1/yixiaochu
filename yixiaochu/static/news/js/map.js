            ...
            return SVGMap.prototype.options = {
                ...
                function sharpHover(a, b) {
                    ...
                    if (opt.external && "undefined" != typeof self.externalData[b.id].eventHoverLock && 0 != self.externalData[b.id].eventHoverLock || -1 == opt.clickedRegion.indexOf(b.id) && b.animate({
                        ...
                        opt.timeTimer = 1, 0 == $("#"+opt.showTipTag).length && $(document.body).append('<div id="'+opt.showTipTag+'" class="'+opt.showTipTag+'"><div class="con"></div><div class="arrow"><div class="arrowMask"></div></div></div'), $("#stateTip .con").html(opt.mapTipHtml(stateData, b));
                ...
                function sharpOut(a, b) {
                    opt.timeTimer = 0, opt.hoverRegion = "", b.timer = setTimeout(function () {
                        ...
                        }, 100), opt.showTip && 1 != opt.timeTimer && $("#"+opt.showTipTag).remove())
                    }, 100)
                }
                ...
                var offsetXY = function (a) {
                        var b, c, d = $("#"+opt.showTipTag).outerWidth(),
                            e = $("#"+opt.showTipTag).outerHeight();
                ...
                for (var state in mapConfig.shapes) {
                    var thisStateData = stateData[state],
                        initColor = thisStateData && opt.stateColorList[thisStateData.stateInitColor] || "#"+mapConfig['names'][state]['color']  || opt.stateInitColor,
                    ...
                    obj.id = state, obj.name =  mapConfig['names'][state]['name'], obj.timer = "";
                   
                    //是否显示地图上文字
                    if(opt.showName){
                    	var s = obj.getBBox().x + mapConfig['names'][state]['x'],
    					offsetY = obj.getBBox().y + mapConfig['names'][state]['y'];
                        obj.font = r.text(s, offsetY, obj.name).attr({
    						cursor: 'pointer',
    						'font-size': mapConfig['names'][state]['font-size'],
    						'font-family': mapConfig['names'][state]['font-family'],
    						'fill': mapConfig['names'][state]['font-color']
    					});
    	 
                    }
                    
                    obj.attr(attributes), opt.external && (self.externalData[obj.id] = obj), stateData[state] && stateData[state].diabled ? obj.attr({
                        fill: disabledColor,
                        cursor: "default"
                    }) : (obj.attr({
                        fill: initColor
