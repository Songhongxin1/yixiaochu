<?php
	class  Valicode {
		 ...
		 */ 
		function __construct($width=100, $height=45, $codeNum=4,$fontPath='') {
			$this->width=$width;                     //为成员属性width初使化
			$this->height=$height;                     //为成员属性height初使化
			$this->codeNum=$codeNum;               //为成员属性codeNum初使化		
			$this->fontPath='../../static/font/CoasterShadow.ttf';		
		...
		public function outImg(){    
           ...
           // $_SESSION["code"]=strtoupper($this->checkCode);  //加到session中
            $CI->session->set_userdata(array('code'  => strtoupper($this->checkCode)));
